...
with st.sidebar:
    selected = option_menu(
        menu_title="💡 기능 선택",
        options=["ROC 커브 비교",  "튜닝 시각화", "통증 예측 시뮬레이션",],
        icons=["bar-chart", "activity", "sliders"],
        menu_icon="cast",
        default_index=0,
        styles={
...
if selected == 'ROC 커브 비교':
    ...
    fig, ax = plt.subplots()
    ax.plot(fpr_lgbm, tpr_lgbm, label=f'LGBM (AUC = {auc_lgbm:.2f})', color='orange', linewidth=2)
    ax.plot(fpr_logreg, tpr_logreg, label=f'LogReg (AUC = {auc_logreg:.2f})', color='royalblue', linewidth=2)
    ax.plot([0, 1], [0, 1], 'k--')
...
elif selected == "튜닝 시각화":
    st.title("🎯 하이퍼파라미터 튜닝 시각화")
    study = load_study()

    st.subheader("1. 최적화 히스토리")
    fig1 = plot_optimization_history(study).figure
    st.pyplot(fig1)

    st.subheader("2. 하이퍼파라미터 중요도")
    fig2 = plot_param_importances(study).figure
    st.pyplot(fig2)

    st.subheader("3. Contour Plot (2개 파라미터 선택)")
    param_list = list(study.best_params.keys())
    selected_params = st.multiselect("두 개를 선택하세요:", param_list, default=param_list[:2])

    if len(selected_params) == 2:
        fig3 = plot_contour(study, params=selected_params).figure
        st.pyplot(fig3)
    else:
        st.info("Contour Plot은 정확히 2개의 파라미터를 선택해야 합니다.")
...
# ------------------------------
    ...
    st.markdown(
        "<h1 style='font-size: 34px;'>🩺 통증 예측 시뮬레이션 (LGBM 모델)</h1>", 
        unsafe_allow_html=True
    )
    ...
    # 직업군 산업군 설명
    occ_ind_labels = {
        "Active": "활동적인 직업 및 산업군(운동선수, 배달원, 청소부)",
        "Mixed": "혼합형 직업 및 산업군(교사, 요리사, 엔지니어)",
        "Sedentary": "앉아서 하는 직업 및 산업군(프로그래머, 회계사)"
    }
